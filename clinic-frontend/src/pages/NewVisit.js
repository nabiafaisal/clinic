...
export default function NewVisit() {
  ...
  const handleDoctorSelect = (doctorId) => {
    ...
    setForm(f => ({ ...f, doctor_id: doctorId, doctor_name: doc ? doc.name : '', doctor_reg_no: doc ? (doc.reg_no || '') : '' }));
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <select className="form-select" value={form.doctor_id} onChange={e => handleDoctorSelect(e.target.value)} required>
                <option value="">— Select doctor —</option>
                {doctors.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
              </select>
              {doctors.length === 0 && (
                <span style={{ fontSize: 11, color: 'var(--ink-lite)', marginTop: 4, display: 'block' }}>
                  No doctors found — add one under Manage Users first.
                </span>
              )}
            </div>
            <div className="form-group">
              <label className="form-label">Registration No.</label>
              <input className="form-input mono" value={form.doctor_reg_no} readOnly placeholder="Auto-filled from doctor" />
            </div>
          </div>
