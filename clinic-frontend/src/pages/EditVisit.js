...
export default function EditVisit() {
  ...
  useEffect(() => {
    ...
    if (isMock) {
      ...
      setForm({
        ...
        delivery_status: '', doctor_id: '', doctor_name: 'Dr. Arshad Mahmood',
        doctor_reg_no: 'PMC-12345', telemedicine_consent: false,
      });
      setDoctors([{ id: 1, name: 'Dr. Arshad Mahmood', reg_no: 'PMC-12345' }]);
      setLoading(false);
      return;
    }
    Promise.all([
  ...
  const handleDoctorSelect = (doctorId) => {
    ...
    setForm(f => ({ ...f, doctor_id: doctorId, doctor_name: doc ? doc.name : f.doctor_name, doctor_reg_no: doc ? (doc.reg_no || '') : f.doctor_reg_no }));
  };
        ...
        <div className="section-card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div className="form-group">
              ...
              <select className="form-input" value={form.doctor_id} onChange={e => handleDoctorSelect(e.target.value)} disabled={locked}>
                <option value="">{form.doctor_name || '— Select doctor —'}</option>
                {doctors.filter(d => d.name !== form.doctor_name).map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label className="form-label">Doctor Reg. No.</label>
              <input className="form-input" value={form.doctor_reg_no} readOnly disabled={locked} placeholder="Auto-filled from doctor" />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div className="form-group">
